...
def main():
    ...
    if st.button("Predict"):
        ...
        if int(Results >= 50 and Results <= 85):
            st.text(f"Your Restaurant may succeed with a percentage: {int(Results)}% of success")
        elif int(Results >= 85):
            st.text(f"Your Restaurant have a great potential to succeed with a percentage of: {int(Results)}% of sucess")
        elif int(Results <= 50 and Results>=25):
            st.text(f"Your Restaurant May Fail with a percentage: {int(Results)}% of success you may reconsider changing some of your features")
        elif int(Results <= 50 and Results>=25):
            st.text(f"Your Restaurant have a less opprtinuty to succeed {int(Results)}% you may reconsider changing some of your features")
        fig = px.histogram(df, y="votes", x='listed_in(city)', color='rate')
        st.plotly_chart(fig)
main()
